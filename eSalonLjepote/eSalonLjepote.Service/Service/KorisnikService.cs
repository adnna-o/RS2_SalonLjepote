...
namespace eSalonLjepote.Service.Service
{
    public class KorisnikService : BaseCRUDService<Model.Models.Korisnik, Database.Korisnik, KorisnikSearchObject, KorisnikInsertRequest, KorisnikUpdateRequest>, IKorisnikService
    {
        ...
        {
            // 1. Dohvati korisnika sa ulogama
            var entity = _context.Korisniks
                                 .Include(x => x.KorisnikUlogas)
                                     .ThenInclude(ku => ku.Uloga)
                                 .FirstOrDefault(x => x.KorisnickoIme == username);

            // 2. Ako ne postoji, vrati null ili baci exception
            if (entity == null)
            {
                // Swagger/Basic auth će vratiti 401
                return null;
            }

            // 3. Generiraj hash unesene lozinke sa saltom iz baze
