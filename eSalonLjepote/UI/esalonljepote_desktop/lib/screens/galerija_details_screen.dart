...
class _GalerijaDetailsScreenState extends State<GalerijaDetailsScreen> {
  ...
  Widget build(BuildContext context) {
    final screenTitle =
        _isEdit ? (widget.galerija!.galerijaId ?? "Galerija") : 'Dodaj galeriju';
  ...
  Future<void> _onSavePressed() async {
    ...
    if (!_isEdit) {
      if (_administratorResult != null &&
          _administratorResult!.result.isNotEmpty) {
        request['administratorId'] =
            _administratorResult!.result.first.administratorId;
      } else {
        _showSnackbar("Nema dostupnog administratora. Kreiranje nije moguće.");
        return;
      }
    }
  ...
  FormBuilder _buildForm() {
    return FormBuilder(
      ...
      initialValue: {
        ...
        if (_isEdit) 'administratorId': widget.galerija?.administratorId?.toString(),
      },
      child: Column(
        children: [
          if (_isEdit)
                  ...
                  child: FormBuilderDropdown<String>(
                    ...
                    items: _administratorResult?.result
                            .map((item) => DropdownMenuItem<String>(
                                  ...
                                  child:
                                      Text(item.korisnik?.ime ?? "Nepoznat"),
                                ))
                            .toList() ??
                        [],
